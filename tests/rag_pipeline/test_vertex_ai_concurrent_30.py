...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f"logs/vertex_ai_concurrent_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"),
...
async def test_concurrent_30_calls():
    ...
    try:
        ...
        # 크레딧 차감 확인 안내
        print(f"\n  💳 API 크레딧 차감 확인:")
        print(f"    1. Google Cloud Console 접속")
        print(f"    2. Vertex AI → Billing 페이지 확인")
        print(f"    3. 또는 다음 명령어로 확인:")
        print(f"       gcloud billing accounts describe YOUR_BILLING_ACCOUNT")
...
async def main():
    ...
    try:
        ...
        print("\n" + "=" * 80)
        if success:
            print("  🎉 테스트 완료! API 크레딧 차감을 Google Cloud Console에서 확인하세요.")
        else:
