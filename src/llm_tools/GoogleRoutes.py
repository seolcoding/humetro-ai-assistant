...
def get_routes(departure: str, destination: str) -> str:
    ...
    def parse_route_to_string(
        departure: str, destination: str, recommened_route: dict
    ) -> str:
        ...
        for step in steps:
            ...
            departure_stop = step["transitDetails"]["stopDetails"]["departureStop"][
                "name"
            ]
            ...
            order_num += 1
        result += (
            f"{order_num}. {arrival_stop}에서 {destination}까지 도보로 이동합니다.\n"
        )
    ...
    def run(departure: str, destination: str) -> str:
        ...
        return result_prefix + parse_route_to_string(
            departure, destination, recommened_route
        )
