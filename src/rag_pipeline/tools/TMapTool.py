...
raise DeprecationWarning("This tool is deprecated, use GoogleRouteTool instead")
    ...
    def parse_directions_dict(self, directions_dict: dict) -> str:
        ...
        for idx, leg in enumerate(legs):
            if leg["mode"] == "SUBWAY":
                ...
                if idx + 1 < len(legs):
                    legs[idx + 1]["start"]["name"] = (
                        legs[idx + 1]["start"]["name"] + "역"
                    )
            elif leg["mode"] == "BUS":
                ...
                if idx - 1 >= 0:
                    legs[idx - 1]["end"]["name"] = (
                        legs[idx - 1]["end"]["name"] + "정류소"
                    )
                if idx + 1 < len(legs):
                    legs[idx + 1]["start"]["name"] = (
                        legs[idx + 1]["start"]["name"] + "정류소"
                    )
        ...
        try:
            ...
            if "result" in TMap_response:
                ...
                if len(itineraries) > 1:
                    ...
                    itineraries.sort(key=lambda x: (x["fare"]["regular"]["totalFare"]))
